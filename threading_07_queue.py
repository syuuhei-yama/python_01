...
def procees_image(file_path):
    #画像の加工
    logging.info('processing = {}'.format(file_path))
    sava_file_path = file_path.split('.')[0] + '_gray.' + file_path.split('.')[1]
    img = Image.open(file_path)
    gray_img = img.convert('L')
    gray_img.save(sava_file_path)
    return sava_file_path

def upload_image(file_path):
    #アップロードする処理
    logging.info('アップロード完了 = {}'.format(file_path))
    return 'https://upload_{}'.format(file_path)
...
download_worker = Worker(
    download_image,download_queue, procees_queue, len(urls)
)
procees_worker = Worker(
    procees_image,procees_queue,upload_queue,len(urls)
)
upload_worker = Worker(
    upload_image, upload_queue, done_queue, len(urls)
)
download_worker.start()
procees_worker.start()
upload_worker.start()
